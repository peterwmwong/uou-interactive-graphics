...
#[test]
fn bindgen_test_layout_LightArg() {
    assert_eq!(
        ::std::mem::size_of::<LightArg>(),
        128usize,
        concat!("Size of: ", stringify!(LightArg))
    );
    assert_eq!(
        ::std::mem::align_of::<LightArg>(),
        16usize,
        concat!("Alignment of ", stringify!(LightArg))
    );
    fn test_field_matrix_world_to_projection() {
        assert_eq!(
            unsafe {
                let uninit = ::std::mem::MaybeUninit::<LightArg>::uninit();
                let ptr = uninit.as_ptr();
                ::std::ptr::addr_of!((*ptr).matrix_world_to_projection) as usize - ptr as usize
            },
            0usize,
