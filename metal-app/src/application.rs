...
static mut RENDERER_PTR: *mut c_void = 0 as _;

#[inline(always)]
fn get_renderer<R: RendererDelgate + 'static>() -> &'static mut MetalRenderer<R> {
    unsafe {
        debug_assert_ne!(
            RENDERER_PTR, 0 as _,
            "Application Renderer Pointer was not initialized! Check from_nswindow()"
        );
    }
    unsafe { &mut *(RENDERER_PTR as *mut MetalRenderer<R>) }
}
...
pub fn from_nswindow<R: RendererDelgate + 'static>(nswindow: *mut Object) -> DisplayLink {
    ...
    unsafe { RENDERER_PTR = &mut *renderer as *mut MetalRenderer<R> as _ };
    init_and_attach_view::<R>(nswindow, &mut renderer);
    init_window_event_handlers::<R>(nswindow);
        ...
        for selector in [
            ...
            decl.add_method(selector, {
                extern "C" fn on_mouse_event<R: RendererDelgate + 'static>(
                    ...
                    get_renderer::<R>().on_event(user_event);
                }
            ...
            extern "C" fn on_key_down<R: RendererDelgate + 'static>(
                ...
                unsafe {
                    ...
                    if key_code == ESCAPE_KEY {
                        ...
                        get_renderer::<R>().on_event(UserEvent::KeyDown {
                            key_code,
...
fn init_window_event_handlers<R: RendererDelgate + 'static>(nswindow: *mut Object) {
    extern "C" fn on_nswindow_resize<R: RendererDelgate + 'static>(
        ...
        get_renderer::<R>().update_size(f32x2::from_array([width as f32, height as f32]));
    }
